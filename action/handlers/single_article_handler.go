...
func GetSingleArticle(c echo.Context, db *sql.DB) error {
	articleID := c.Param("id")
	log.Printf("Requête pour l'article ID : %s", articleID)

	// Requête SQL principale pour récupérer l'article
	query := `
        SELECT 
	...
	if err == sql.ErrNoRows {
		log.Printf("Article avec ID %s introuvable", articleID)
...
func GetSingleArticleFactices(c echo.Context) error {
	id := c.Param("id")
